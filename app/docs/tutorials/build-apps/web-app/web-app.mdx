...
Parameters for other networks are available on the [network variables](network-variables.md) page.
...
Use the [API page](../api/api.md) to get the list of all supported transactions (the transactions are started from `Msg`
...
minted NFTs or the NFT owner, for example. Use the [API page](../api/api.md) to get the list of all supported queries
(the queries are in the `Query` proto services and located in the `query.proto` files).

#### Extend Coreum transactions and queries.

The [Using CosmJS#](cosmjs.md) explains how to generate the TypeScript code from the protos, here for the simplicity
...
* In some cases you might need the data that the node doesn't return, in that case, you can check the
  [Explorer API](explorer-api.md) to understand what additionally you can get from the indexed data.

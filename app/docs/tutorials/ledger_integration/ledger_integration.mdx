...
* Set up [Ledger Live](https://www.ledger.com/ledger-live/) on your machine.

* The Cosmos(ATOM) application must be installed on your Ledger device, for more information click [here](https://support.ledger.com/hc/en-us/articles/360013713840-Cosmos-ATOM-?support=true). (Make sure that Cosmos(ATOM) app version is v2.34.12 or above)

* `cored` ([Coreum CLI](cored.md)) installed on your local machine (install cored binary version v1.0.0 or above)
...
- If you want to target other network than testnet, replace it with values at [network variables page](../tutorials/network-variables.md)
:::
...
* Fund your account address
  ...
  * Navigate [faucet page](../tools-ecosystem/faucet.md) designed specifically for this purpose.
...
* Connect to the [Coreum Chain](../tools-ecosystem/wallet.md#keplr) to add the Coreum account.
  :::details OPTIONAL
...
* Click the `Connect to coreum-testnet-1` button to establish a connection with the Coreum testnet chain.
  ...
  listed on [Coreum Docs page](../tools-ecosystem/wallet.md#keplr).
  :::
...
* To fund the test account:
  ...
  * Open the [faucet](../tools-ecosystem/faucet.md#testnet) and paste the copied address into the designated address field.

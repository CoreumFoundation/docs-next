...
```bash
RES=$(cored tx wasm store artifacts/ft_extension.wasm \
...
* extension_instantiation_msg - optional json encoded data to pass to WASM on instantiation
...
In this special case, we want to receive denom of the parent token if there is any and also store it, so we need to add
a expand `UserProvidedInstantiationMsg` struct and also add a new state item.
...
```bash
cored tx assetft issue MYFT2 cmyft2 2 1000000 "My second FT token with extension" --from $FT_ADMIN --features=whitelisting,extension --extension_code_id=$CODE_ID --extension_label=my-second-extension --extension_instantiation_msg='{"parent_denom": "'$FT_DENOM'"}' $COREUM_NODE_ARGS $COREUM_CHAIN_ID_ARGS -y -b block --gas auto --gas-adjustment 1.4
```

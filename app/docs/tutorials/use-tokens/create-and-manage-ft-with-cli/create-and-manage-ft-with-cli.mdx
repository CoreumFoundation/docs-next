...
This tutorial will guide you through the process of using the <a href="/docs/modules/assetft" target="_blank" rel="noopener noreferrer" class="text-[#24cc8e] hover:text-[#1db07a] text-md font-bold">assetft</a>
...
-   Since we are going to use the Testnet network in this tutorial, let's export its variables:
    ...
    <a href="network-variables.md" target="_blank" rel="noopener noreferrer" class="text-[#24cc8e] hover:text-[#1db07a] text-md font-bold">network variables</a>
 page.

-   <a href="/docs/tutorials/get-started/install-cored" target="_blank" rel="noopener noreferrer">Install cored binary</a>

-   You should have two accounts on your local keychain. If you do not have them, you can go to the
    <a href="/docs/tools/faucet" target="_blank" rel="noopener noreferrer" class="text-[#24cc8e] hover:text-[#1db07a] text-md font-bold">faucet page</a>
 and generate funded accounts for the testnet.
...
```bash
  ...
  cored tx assetft issue MYFT cmyft 2 100 "My first FT token" --from $FT_ISSUER --features=burning,freezing,minting,whitelisting --send-commission-rate=0.02 --node=$RPC_URL --chain-id=$CHAIN_ID
  # where:
  ...
  Copy this hash and go to the <a href="/docs/tools/blockchain-explorers" target="_blank" rel="noopener noreferrer" class="text-[#24cc8e] hover:text-[#1db07a] text-md font-bold">Block explorer</a>
 to see the transaction's status.
...
-   Since we have enabled the `minting` feature, the token issuer can mint additional tokens by
...
performed on them by anyone except the issuer until they are unfrozen using the `globally-unfreeze`
command.
...
behalf of the common account. Even sending tokens to the issuer will not be possible, as the coins
...
Note that even after the global freeze, the issuer of the tokens will still be able to perform
...
except when one of the participants is the token issuer. To illustrate how this works, follow the
steps below:

-   First, we need a third account to transfer tokens to, let's
    <a href="/docs/tools/faucet" target="_blank" rel="noopener noreferrer" class="text-[#24cc8e] hover:text-[#1db07a] text-md font-bold">generate</a>
 it and prepare:
    ```bash
...
-   Let's check account balances:
    ...
    applied, and 2 tokens were transferred to the token issuer.

## Burn-rate

The `burn-rate` works similarly to the <a href="/docs/overview/smart-tokens" target="_blank" rel="noopener noreferrer" class="text-[#24cc8e] hover:text-[#1db07a] text-md font-bold">send-commission-rate</a>. The
only difference is that when the `burn-rate` is applied, the token issuer does not receive any
...
You can read more about Fungible Tokens at <a href="/docs/modules/coreum-fungible-token" target="_blank" rel="noopener noreferrer" class="text-[#24cc8e] hover:text-[#1db07a] text-md font-bold">Smart Token Spec</a>
 pages

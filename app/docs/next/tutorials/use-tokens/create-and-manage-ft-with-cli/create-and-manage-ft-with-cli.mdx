...
-   You should have two accounts on your local keychain. If you do not have them, you can go to the
    [faucet page](/docs/next/tools/faucet) and generate funded accounts for the testnet.
...
```bash
  ...
  Copy this hash and go to the [Block explorer](/docs/next/tools/blockchain-explorers) to see the transaction's status.
...
-   First, we need a third account to transfer tokens to, let's [generate](/docs/next/tools/faucet) it and prepare:
    ```bash

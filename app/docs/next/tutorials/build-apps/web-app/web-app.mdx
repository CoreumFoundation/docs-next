...
Parameters for other networks are available on the [network variables](/docs/next/nodes-and-validators/essentials/network-variables) page.
...
Use the [API page](/docs/next/api/protobuf)  to get the list of all supported transactions (the transactions are started from `Msg`
...
minted NFTs or the NFT owner, for example. Use the [API page](/docs/next/api/protobuf) to get the list of all supported queries
(the queries are in the `Query` proto services and located in the `query.proto` files).

#### Extend Coreum transactions and queries.

The [Using CosmJS#](/docs/next/tutorials/get-started/wasm/using-cosmjs-with-wasm-contracts) explains how to generate the TypeScript code from the protos, here for the simplicity
...
* Read Coreum [modules](/docs/next/modules) specification, to be familiar with the custom Coreum functionality you can
  use for your
  application.
...
* In some cases you might need the data that the node doesn't return, in that case, you can check the
  [Explorer API](/docs/next/tools-and-ecosystem/blockchain-explorers) to understand what additionally you can get from the indexed data.

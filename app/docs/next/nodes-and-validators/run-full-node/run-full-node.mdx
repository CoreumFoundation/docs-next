...
* [Check requirements](/docs/next/become-validator/essentials/system-requirements)

* [Set up node prerequisites](/docs/next/become-validator/essentials/node-prerequisites)

* [Install cored binary](/docs/next/become-validator/essentials/install-cored).

* Verify that [network variables](/docs/next/become-validator/essentials/network-variables) are set up correctly.
...
* Set the common connection config using the [doc](/docs/next/become-validator/essentials/connection-config).
...
* (Optional) Enable REST APIs disabled by default.
  ...
  ```
  If you face `File contains parsing errors`  issue, check [troubleshooting section](/docs/next/become-validator/troubleshooting)
...
* Enable state-sync, so your validator can sync by snapshot.
  ...
  instead of state-sync you should visit <a href="/docs/next/become-validator/upgrades/history" className="text-[#24D494] font-bold">this section</a>

  You can read <a href="https://docs.tendermint.com/v0.34/tendermint-core/state-sync.html" target="_blank" rel="noopener noreferrer" className="text-[#24D494] font-bold">Using State Sync</a> document to get more details.

  ** Get at least two state sync servers and set them from the [doc](/docs/next/become-validator/essentials/network-variables):
  ```bash
...
You can run [validator node](/docs/next/become-validator/validator/run-validator)

...
* `order_id` - unique order identifier of the order. the order id matches against regular expression `^[a-zA-Z0-9/+:._-]{1,40}$`.
...
which is determined by DEX governance. The default value is 100 but the parameter can be change with governance proposal, to check the current value visit [block explorer](https://explorer.devnet-1.coreum.dev/coreum/params).

### Events

The DEX module emits events at the time of the matching to notify the interested parties of the changes caused by the
matching. The events are defined in the <a target="_blank" href="https://github.com/CoreumFoundation/coreum/blob/master/proto/coreum/dex/v1/event.proto" rel="noopener noreferrer" className="text-[#25D695] hover:opacity-80 text-base font-bold">event.proto</a>. The events are emitted
...
The order sequence is a unique identifier assigned to each order within the system. It is used to maintain the order's position in the order book and ensure proper ordering and execution of trades. The sequence is critical for tracking, managing, and retrieving orders efficiently.
...
- [`events.proto`](https://github.com/CoreumFoundation/coreum/blob/master/proto/coreum/dex/v1/event.proto)
- [`tx.proto`](https://github.com/CoreumFoundation/coreum/blob/master/proto/coreum/dex/v1/tx.proto)
- [`query.proto`](https://github.com/CoreumFoundation/coreum/blob/master/proto/coreum/dex/v1/query.proto)
- [`params.proto`](https://github.com/CoreumFoundation/coreum/blob/master/proto/coreum/dex/v1/params.proto)
...
* (Optional) If you want your validator to be a state sync server (provide state snapshots for other nodes), you can enable it:
  ...
  ```
 You can read <a href="https://docs.tendermint.com/v0.34/tendermint-core/state-sync.html" target="_blank" rel="noopener noreferrer" style={{ color: '#24D494', fontWeight: 'bold' }}>Using State Sync</a> document to get more details.
...
* Enable state-sync, so your validator can sync by snapshot. 
  ...
  instead of state-sync you should visit <a href="/docs/become-validator/upgrades/history" target="_blank" rel="noopener noreferrer" style={{ color: '#24D494', fontWeight: 'bold' }}>this section</a>

...
* (Optional) If you want your validator to be a state sync server (provide state snapshots for other nodes), you can enable it:
  ...
  ```
  You can read [Using State Sync](https://docs.tendermint.com/v0.34/tendermint-core/state-sync.html) document to get more details.
...
* Enable state-sync, so your validator can sync by snapshot. 
  ...
  instead of state-sync you should visit [this section](/docs/become-validator/upgrades/history)

...
* Enable state-sync, so your validator can sync by snapshot. 
  ...
  instead of state-sync you should visit [this section](upgrade-history.md)

  You can read [Using State Sync](https://docs.tendermint.com/v0.34/tendermint-core/state-sync.html) document to get more details.

  ** Get at least two state sync servers and set them from the [doc](network-variables.md):
  ```bash

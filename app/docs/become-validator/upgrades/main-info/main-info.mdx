...
export NEW_CORED_VERSION="v4.0.0"
export UPGRADE="v4"
...
The printed version should match `v4.0.0`. If the version doesn't match stop following this tutorial immediately!
...
The printed version should match `v4.0.0`. If the version doesn't match, go to previous step and make sure you have
downloaded the correct binary! Continuing will lead to serious problems.

* Verify that the `cored` binary is executable by the user you used to run cosmovisor.

* Make sure that old binary version is correct, the following command should print `v4.0.0`
```bash
